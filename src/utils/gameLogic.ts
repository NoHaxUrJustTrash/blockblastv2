...
export const clearLines = (
  grid: (string | null)[][]
): { updatedGrid: (string | null)[][], clearedLines: number, clearedPositions: {row: number, col: number}[] } => {
  const gridSize = grid.length;
  let clearedLines = 0;
  ...
  for (let col = 0; col < gridSize; col++) {
    if (Array.from({ length: gridSize }, (_, row) => updatedGrid[row][col]).every(cell => cell !== null)) {
      // Clear the column
      for (let row = 0; row < gridSize; row++) {
        if (updatedGrid[row][col] !== null) { // Only count if not already cleared by row
          clearedPositions.push({row, col});
        }
        updatedGrid[row][col] = null;
      }
  ...
  return { updatedGrid, clearedLines, clearedPositions };
};
